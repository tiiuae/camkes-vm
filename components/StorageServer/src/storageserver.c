...
/* Prototypes for functions that are not generated by the CAmkES yet.
 * These are provided by the StorageServer templates.
 */
...
{
    return client_get_sender_id();
}

static inline seL4_Word get_num_badges(void)
{
    return client_num_badges();
}

static inline seL4_Word enum_client_badge(seL4_Word id)
{
    return client_enumerate_badge(id);
}

static inline void* get_client_buf(seL4_Word id)
{
    return client_buf(id);
}

static inline void get_client_partitions(seL4_Word badge, uint8_t *part_list, uint8_t *num_parts)
{
    client_get_partitions(badge, part_list, num_parts);
}
