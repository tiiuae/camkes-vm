...
static int vmm_init(void)
{
    ...
    allocman = bootstrap_use_current_1level(
                   ...
                   ALLOCATOR_MEMPOOL_SZ, allocator_mempool
               );
    assert(allocman);
...
static int alloc_vm_device_cap(uintptr_t addr, vm_t *vm, vm_frame_t *frame_result)
{
    ...
    seL4_Word cookie;
    err = vka_utspace_alloc_at(vm->vka, &frame, kobject_get_type(KOBJECT_FRAME, seL4_PageBits), seL4_PageBits, addr, &cookie);
    if (err) {
