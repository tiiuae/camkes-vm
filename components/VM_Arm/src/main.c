...
static int load_linux(vm_t *vm, const char *kernel_name)
{
    ...
    if (linux_image_config.dtb_generate) {
        void *fdt_ori;
        void *gen_fdt = linux_gen_dtb_buf;
        int size_gen = PLAT_LINUX_DTB_SIZE;
        int num_paths = 0;
        ...
        if (&vm_image_dtb) {
            vm_guest_ram_read(vm, vm_image_dtb.addr, linux_gen_dtb_base_buf, size_gen);
            fdt_ori = (void *)linux_gen_dtb_base_buf;
        } else {
            camkes_io_fdt(&(_io_ops.io_fdt));
            fdt_ori = (void *)ps_io_fdt_get(&_io_ops.io_fdt);
        ...
        if (&vm_image_dtb) {
            ZF_LOGI("Using autogenerated dtb @ 0x"SEL4_PRIx_word" based on %s",
                    dtb_addr, vm_image_dtb.filename);
        } else {
            ZF_LOGI("Using autogenerated dtb @ 0x"SEL4_PRIx_word, dtb_addr);
        }
    } else {
        if (&vm_image_dtb) {
            ZF_LOGI("Using dtb @ 0x"SEL4_PRIx_word" from %s", dtb_addr,
                    vm_image_dtb.filename);
        } else {
            ZF_LOGW("No dtb specified nor autogenerated");
        }
    }

    /* Set boot arguments */
    err = vcpu_set_bootargs(vm->vcpus[BOOT_VCPU], entry, MACH_TYPE, dtb_addr);
    if (err) {
        printf("Error: Failed to set boot arguments\n");
        return -1;
    }
